...
int process_input(char* inp, char* out) {
    ...
    // null terminating string. (asciiz)
    ...
    while(end_of_sentence == 0 && word_found == 0) {
        ...
        is_delim_ch = is_delimiting_char(cur_char);   // Check if it is a delimiting character

        if (is_delim_ch == 1) {  //throwing away other delimiting characters
...
int main() {
    ...
    while(1) {    //Infinite loop, as many times as the user wants to process the information (very large file)
